...
class RabbitMQWriteOperationsAuditIntegrationTest extends IntegrationTestBase {
    ...
    class AuditLoggingIntegration {
        ...
        void createExchange_ShouldLogAuditTrail() throws Exception {
            ...
            verify(rabbitMQResourceService).createExchange(eq(testCluster.getId()), eq(request), any(User.class));
        }
    ...
    class MetricsCollectionIntegration {
        ...
        void failedOperations_ShouldCollectMetrics() throws Exception {
            ...
            // layer)
            verify(rabbitMQResourceService).createQueue(eq(testCluster.getId()), eq(request), any(User.class));
        }
    ...
    class ErrorHandlingAndLogging {
        ...
        void pathVariableDecodingErrors_ShouldBeLoggedAndHandled() throws Exception {
            ...
            verify(rabbitMQResourceService, never()).createBinding(any(), any(), any(), any(), any(), any(), any());
        }
    ...
    class SecurityAndAuthorizationLogging {
        ...
        void writeOperations_ShouldLogUserAccessPatterns() throws Exception {
            ...
            when(rabbitMQResourceService.createExchange(eq(testCluster.getId()), eq(exchangeRequest), any(User.class)))
                    .thenReturn(Mono.empty());
            when(rabbitMQResourceService.createQueue(eq(testCluster.getId()), eq(queueRequest), any(User.class)))
                    .thenReturn(Mono.empty());
            ...
            verify(rabbitMQResourceService).createExchange(eq(testCluster.getId()), eq(exchangeRequest),
                    any(User.class));
            verify(rabbitMQResourceService).createQueue(eq(testCluster.getId()), eq(queueRequest), any(User.class));
        }
        ...
        void clusterAccess_ShouldBeLogged() throws Exception {
            ...
            mockMvc.perform(put("/api/rabbitmq/{clusterId}/resources/exchanges", nonExistentClusterId)
                    ...
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(objectMapper.writeValueAsString(request)))
                    .andExpect(status().isInternalServerError()); // Service will handle cluster not found

            // Then - Verify service was called (cluster validation happens in service
            // layer)
            verify(rabbitMQResourceService).createExchange(eq(nonExistentClusterId), eq(request), any(User.class));
        }
    }
