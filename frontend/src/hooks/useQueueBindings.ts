...
interface UseQueueBindingsState {
  bindings: RabbitMQBinding[] | null;
  loading: boolean;
  error: ResourceError | null;
}

interface UseQueueBindingsActions {
  loadBindings: (clusterId: string, vhost: string, queueName: string) => Promise<void>;
  clearError: () => void;
}

export const useQueueBindings = (): UseQueueBindingsState & UseQueueBindingsActions => {
  const [state, setState] = useState<UseQueueBindingsState>({
    bindings: null,
    loading: false,
    error: null
  });
  ...
  const loadBindings = useCallback(async (
    ...
    try {
      ...
      const response = await rabbitmqResourcesApi.getQueueBindings(clusterId, vhost, queueName);
      setBindings(response);
    } catch (err: any) {
      console.error('Error loading queue bindings:', err);
